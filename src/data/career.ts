import { Career } from "@/types";
import type { Locale } from "@/context/LanguageContext";

const careersKo: Career[] = [
  {
    company: "주식회사 휴니크",
    role: "기업부설연구소 연구소장 · 팀장",
    period: "2025.01 ~ 현재",
    duration: "재직 중",
    descriptions: [
      "무인카페 Mailbean 플랫폼 총괄 — 보드 펌웨어부터 Electron 키오스크, SaaS 백엔드, 브랜드 사이트까지 수직 통합",
      "Django 모놀리스(views.py 1.4만 줄)를 Java 21 헥사고날 MSA 19모듈로 스트랭글러 전환, NCP→AWS 무중단 마이그레이션",
      "커피머신 제어보드 펌웨어 hex 역분석 → ATmega2560 클린 재작성, 실보드 검증",
      "오픈소스 Erupt 기반 로우코드 플랫폼 HunikFlow 고도화 — ERP 9모듈·MES 3모듈·AI 모듈 확장",
      "CS 응대 AI 개발 — 해피톡·비즈톡·OpenAI·Notion·Slack 연동 아키텍처/시나리오/DB 설계",
      "Claude Code Skills/Plugin 기반 AI 네이티브 개발 체계 도입",
    ],
    techStack: [
      "Java 21",
      "Spring Cloud",
      "Electron",
      "React",
      "AWS",
      "Terraform",
      "Claude Code",
    ],
  },
  {
    company: "코베아그룹",
    role: "개발팀 부장 · 본부장",
    period: "2022.06 ~ 2024.11",
    duration: "2년 6개월",
    descriptions: [
      "우즈베키스탄 코인 거래소 개발/운영 PM",
      "2.0 버전 신규 개발 — 전체 소스 리팩토링, DB 설계",
      "CI/CD 구축 (GitLab, Jenkins), 배포 프로세스 설계",
      "우즈베키스탄 은행·정부 로그인 연동 (라보나크 은행 / One ID)",
      "Master Card & Visa Card 결제 연동",
      "Spring Cloud MSA 구성 (Eureka, Ribbon)",
      "Kafka 비동기 메시지, Prometheus/Grafana 모니터링",
    ],
    techStack: [
      "Java",
      "Node.js",
      "MySQL",
      "Kafka",
      "Redis",
      "MongoDB",
      "Spring Cloud",
    ],
  },
  {
    company: "코어닥스",
    role: "개발팀 팀장",
    period: "2020.03 ~ 2022.03",
    duration: "2년 1개월",
    descriptions: [
      "거래소 Frontend(Vue.js) / Backend(Java) 개발 및 PM",
      "REST API 개발 (Java, JWT, Spring, Swagger)",
      "ISMS-P 인증 대응 (전문 암호화, 배포 프로세스 설계)",
      "거래소 매칭 서버 유지보수 (C/C++)",
      "모바일 앱 개발 (Flutter) PM",
    ],
    techStack: ["Vue.js", "Java", "Spring", "Flutter", "C/C++"],
  },
  {
    company: "언더핀",
    role: "개발팀 팀장",
    period: "2019.08 ~ 2020.01",
    duration: "6개월",
    descriptions: [
      "PM 및 Flutter 기반 앱·API 서버 개발",
      "Fanzy 앱 개발",
    ],
    techStack: ["Dart", "Flutter", "Java", "Spring"],
  },
  {
    company: "와이온",
    role: "개발 2팀 차장",
    period: "2016.11 ~ 2019.08",
    duration: "2년 10개월",
    descriptions: [
      "간호평가원 평가 시스템 PM 및 개발",
      "인도네시아 키오스크 서버 개발 (텔콤셀/FINNET 결제 연동)",
      "DOKU(인도네시아 PG사) 서버 연동 API 연구 개발",
      "Oracle DB + AWS 인프라 (Load Balancing, KMS, RDS)",
      "ERC20·스텔라 토큰 개발",
    ],
    techStack: ["Java", "Oracle", "AWS", "JPA", "Hibernate", "Blockchain"],
  },
  {
    company: "키위소프트",
    role: "개발팀 팀원",
    period: "2015.12 ~ 2016.11",
    duration: "1년",
    descriptions: [
      "금수저 하이브리드 앱 개발",
      "WEB 전체 개발, API·CMS 일부 개발",
    ],
    techStack: ["AWS", "MySQL", "Spring MVC", "Java", "KT Cloud"],
  },
  {
    company: "actived&c",
    role: "개발 1팀 팀원",
    period: "2013.10 ~ 2015.12",
    duration: "2년 3개월",
    descriptions: [
      "MDMS 빅데이터 분석 사이트 (전력 관련)",
      "스마트빌더 웹사이트 구축",
      "한컴핀테크 사이트 결제·프론트 개발 (PL)",
    ],
    techStack: ["Java", "Spring MVC", "JavaScript", "AWS", "LG 결제"],
  },
  {
    company: "슈어컴",
    role: "개발팀",
    period: "2012.12 ~ 2013.05",
    duration: "6개월",
    descriptions: ["SKT 통신 예측 프로그램 개발"],
    techStack: ["Java"],
  },
  {
    company: "나인휠스",
    role: "개발팀 팀원",
    period: "2011.08 ~ 2012.07",
    duration: "1년",
    descriptions: [
      "그레이프 밸리 (GM툴, 통계툴, 서버 친구 기능, 웹)",
      "마법퀴즈전 (GM툴, 웹)",
    ],
    techStack: ["Java", "IIS", "SQL"],
  },
  {
    company: "케이사인",
    role: "PKI 사업부",
    period: "2010.05 ~ 2011.07",
    duration: "1년 3개월",
    descriptions: ["NPKI 보안 프로토콜 개발"],
    techStack: ["Security", "PKI"],
  },
  {
    company: "뷰아이텍",
    role: "부설 연구소",
    period: "2009.03 ~ 2010.03",
    duration: "1년 1개월",
    descriptions: ["애니뷰/애시뷰 개발·유지보수 (MFC, 임베디드용 가상키보드)"],
    techStack: ["MFC", "C++", "Embedded"],
  },
  {
    company: "케이아이티",
    role: "ITS사업부",
    period: "2006.09 ~ 2008.12",
    duration: "2년 4개월",
    descriptions: [
      "유진 ERP — 차량관제 시스템 통신서버 개발 (Java/Oracle)",
      "삼표레미콘 차량관제 시스템 통신서버 (MFC/MSSQL)",
      "KIT 본사 MMS/SMS 송신서버 관리·유지보수 (C/MFC)",
      "SK C&C 캐쉬백 서버 개발 (UNIX C)",
    ],
    techStack: ["Java", "Oracle", "MFC", "MSSQL", "UNIX C"],
  },
];

const careersEn: Career[] = [
  {
    company: "Hunik Inc.",
    role: "R&D Lab Director · Team Lead",
    period: "2025.01 ~ Present",
    duration: "Current",
    descriptions: [
      "Own the Mailbean unmanned-café platform end to end — board firmware, Electron kiosk, SaaS backend, brand site",
      "Strangler migration of a Django monolith (14k-line views.py) to a 19-module Java 21 hexagonal MSA; zero-downtime NCP→AWS move",
      "Coffee-machine control-board firmware: hex reverse engineering → ATmega2560 clean rewrite, verified on real hardware",
      "Extended the open-source Erupt low-code engine into HunikFlow — added ERP, MES & AI modules",
      "AI-powered CS agent — HappyTalk · BizTalk · OpenAI · Notion · Slack architecture, scenario & DB design",
      "Introduced an AI-native development practice built on Claude Code Skills/Plugins",
    ],
    techStack: ["Java 21", "Spring Cloud", "Electron", "React", "AWS", "Terraform", "Claude Code"],
  },
  {
    company: "Kobea Group",
    role: "Dev Dept. Director",
    period: "2022.06 ~ 2024.11",
    duration: "2y 6m",
    descriptions: [
      "Uzbekistan crypto exchange development/operations PM",
      "Full v2.0 rebuild — source refactoring, DB redesign",
      "CI/CD setup (GitLab, Jenkins), deployment process design",
      "Uzbekistan bank & gov login integration (Ravnaq Bank / One ID)",
      "Mastercard & Visa card payment integration",
      "Spring Cloud MSA (Eureka, Ribbon)",
      "Kafka async messaging, Prometheus/Grafana monitoring",
    ],
    techStack: ["Java", "Node.js", "MySQL", "Kafka", "Redis", "MongoDB", "Spring Cloud"],
  },
  {
    company: "Coredax",
    role: "Dev Team Lead",
    period: "2020.03 ~ 2022.03",
    duration: "2y 1m",
    descriptions: [
      "Exchange Frontend (Vue.js) / Backend (Java) development & PM",
      "REST API development (Java, JWT, Spring, Swagger)",
      "ISMS-P certification (encryption, deployment process)",
      "Exchange matching engine maintenance (C/C++)",
      "Mobile app PM (Flutter)",
    ],
    techStack: ["Vue.js", "Java", "Spring", "Flutter", "C/C++"],
  },
  {
    company: "Underfin",
    role: "Dev Team Lead",
    period: "2019.08 ~ 2020.01",
    duration: "6m",
    descriptions: ["PM & Flutter app + API server development", "Fanzy app development"],
    techStack: ["Dart", "Flutter", "Java", "Spring"],
  },
  {
    company: "Wion",
    role: "Senior Developer",
    period: "2016.11 ~ 2019.08",
    duration: "2y 10m",
    descriptions: [
      "Nursing assessment system PM & development",
      "Indonesia kiosk server (Telkomsel/FINNET payment integration)",
      "DOKU (Indonesian PG) server integration R&D",
      "Oracle DB + AWS infra (Load Balancing, KMS, RDS)",
      "ERC20 & Stellar token development",
    ],
    techStack: ["Java", "Oracle", "AWS", "JPA", "Hibernate", "Blockchain"],
  },
  {
    company: "KiwiSoft",
    role: "Developer",
    period: "2015.12 ~ 2016.11",
    duration: "1y",
    descriptions: ["Hybrid app development", "Full WEB development, partial API & CMS"],
    techStack: ["AWS", "MySQL", "Spring MVC", "Java", "KT Cloud"],
  },
  {
    company: "Actived&C",
    role: "Developer",
    period: "2013.10 ~ 2015.12",
    duration: "2y 3m",
    descriptions: [
      "MDMS big data analysis site (power sector)",
      "SmartBuilder website",
      "Hancom Fintech payment & frontend (PL)",
    ],
    techStack: ["Java", "Spring MVC", "JavaScript", "AWS", "LG Payment"],
  },
  {
    company: "SureCom",
    role: "Developer",
    period: "2012.12 ~ 2013.05",
    duration: "6m",
    descriptions: ["SKT telecom prediction program development"],
    techStack: ["Java"],
  },
  {
    company: "Nine Wheels",
    role: "Developer",
    period: "2011.08 ~ 2012.07",
    duration: "1y",
    descriptions: [
      "Grape Valley (GM tools, stats tools, server features, web)",
      "Magic Quiz (GM tools, web)",
    ],
    techStack: ["Java", "IIS", "SQL"],
  },
  {
    company: "KSign",
    role: "PKI Division",
    period: "2010.05 ~ 2011.07",
    duration: "1y 3m",
    descriptions: ["NPKI security protocol development"],
    techStack: ["Security", "PKI"],
  },
  {
    company: "ViewiTech",
    role: "R&D Lab",
    period: "2009.03 ~ 2010.03",
    duration: "1y 1m",
    descriptions: ["AnyView/AshView development (MFC, embedded virtual keyboard)"],
    techStack: ["MFC", "C++", "Embedded"],
  },
  {
    company: "KIT",
    role: "ITS Division",
    period: "2006.09 ~ 2008.12",
    duration: "2y 4m",
    descriptions: [
      "Yujin ERP — vehicle tracking server (Java/Oracle)",
      "Sampyo Remicon tracking server (MFC/MSSQL)",
      "KIT HQ MMS/SMS server management (C/MFC)",
      "SK C&C cashback server (UNIX C)",
    ],
    techStack: ["Java", "Oracle", "MFC", "MSSQL", "UNIX C"],
  },
];

export const careers = careersKo;
export function getCareers(locale: Locale) {
  return locale === "en" ? careersEn : careersKo;
}
